/**
 * getdocfile.js
 * Matches lesson_chunk table columns:
 *   id, file_name, chunk_index, content, embedding, img_url
 */
...
// ─────────────────────────────────────────────────────────────────────────────
// Build chunks, collecting ALL image URLs within each chunk
// ─────────────────────────────────────────────────────────────────────────────
function buildChunksWithImages(orderedBlocks) {
  ...
  for (const block of orderedBlocks) {
    if (block.type === 'image') {
      ...
      while (textBuffer.length > CHUNK_SIZE) {
        ...
        if (content) {
          result.push({ content, img_urls: [...imgSet] })
        }
  ...
  if (textBuffer.trim()) {
    result.push({ content: textBuffer.trim(), img_urls: [...imgSet] })
  }
...
export async function ingestDocument(fileBuffer, fileName, fileType) {
  ...
  if (fileType === 'txt') {
    ...
    await insertChunks(
      chunks.map((content, i) => ({ content, img_urls: [], embedding: embeddings[i] })),
      fileName
    )
  ...
  for (const block of orderedBlocks) {
    if (block.type === 'image') {
      ...
      imageCount++
      console.log(`[ingest]   ✅ ${block.name} → ${block.url}`)
    }
  }
  console.log(`[ingest] ${imageCount} image(s) uploaded.`)
...
async function insertChunks(chunks, fileName) {
  for (let i = 0; i < chunks.length; i += DB_BATCH_SIZE) {
    ...
    const rows  = batch.map((c, j) => ({
      file_name:   fileName,                        // ← your column
      chunk_index: i + j,                           // ← your column
      content:     c.content,                       // ← your column
      embedding:   c.embedding,                     // ← your column
      img_url:     c.img_urls.join(', ') || null,   // ← your column (text, so join array with comma)
    }))
